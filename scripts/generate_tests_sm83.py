...
for info in SUPPORTED_OPCODES:
    ...
    with open(input_filename, "r") as input_fn, open(output_filename, "w", encoding="utf-8") as output_fn:
        ...
        if prefix:
            output_fn.write(f"#include <cpu/include/instruction_prefix.hpp>\n")
...
""")
    ...
    {{
        ...
        auto ticks = instruction::ExecuteActiveDecoder(static_cast<const Byte&>(resultByte), mmap);
        ...
        TEST_ASSERT_(resultByte.IsSuccess(), "%s", resultByte.GetStatusDescription());
        auto ticks2 = instruction::ExecuteActiveDecoder(static_cast<const Byte&>(resultByte2), mmap);
        TEST_ASSERT(ticks == instruction::{registry_string}::Ticks[static_cast<const Byte&>(resultByte2)]);
